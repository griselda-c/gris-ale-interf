...
public class JugarPage extends WebPage {
	...
	public ListView<Apuesta> apuestasAnterioresComponente;
	...
	public void configurarSelectApuestas() {			
		...
		selectApuestaVistaComponete = new DropDownChoice<OpcionJugada>("selectApuestaVista", new PropertyModel<OpcionJugada>(this.model, "apuestaVista"), this.model.getJugadasPosibles(), new ChoiceRenderer<OpcionJugada>("nombre")) ;
	...
	public void configurarListadoApuestasPartidaAnterior() {	
		...
		// TODO controlar al girar la ruleta que se actualicen las apuestas de la partida anterior
		
		apuestasAnterioresComponente = new ListView<Apuesta>("apuestasAnteriores", new PropertyModel<List<Apuesta>>(this.model, "apuestasPartida")) {
		@Override
		protected void populateItem(ListItem<Apuesta> item) {
			item.add(new Label("tipoApuesta", new PropertyModel<String>(item.getModel(), "jugador.nombre")));				
			item.add(new Label("jugadaApuesta", new PropertyModel<String>(item.getModel(), "tipoApuesta")));				
			item.add(new Label("fichasApuesta", new PropertyModel<Integer>(item.getModel(), "fichas")));					
			item.add(new Label("pagaApuesta", new PropertyModel<Integer>(item.getModel(), "fichas")));				
			}
		};
